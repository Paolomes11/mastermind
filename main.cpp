...
static BenchStats benchmark_strategy(const GameConfig& cfg,
    ...
    r.mean = mean;
    double var = 0;
    for (int t : counts) { double d = t - mean; var += d * d; }
    ...
    for (int t : counts) ++r.dist[t];
    return r;
}

static void run_benchmark(const GameConfig& cfg, const FeedbackTable& fb_table,
                           bool verbose) {
    ...
    for (const auto& name : names) {
        ...
        std::cout << "  done in " << static_cast<int>(results.back().second.elapsed_ms)
                  << " ms\n";
    }

    std::cout << "\n=== Benchmark: " << cfg.description() << " ===\n\n"
              << std::left  << std::setw(10) << "Strategy"
    ...
    for (const auto& [name, s] : results) {
        std::cout << std::left  << std::setw(10) << name
                  ...
                  << std::setw(6) << s.min_t
                  << std::setw(6) << s.max_t
                  << std::setw(10) << static_cast<int>(s.elapsed_ms) << "\n";
    }

    // Distribution
    int max_t = 0;
    for (const auto& [_, s] : results) if (s.max_t > max_t) max_t = s.max_t;

    std::cout << "\n" << std::left << std::setw(8) << "Turns";
    for (const auto& [name, _] : results) std::cout << std::right << std::setw(10) << name;
...
int main(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "c:p:s:bivh", long_opts, nullptr)) != -1) {
        ...
        case 's': strategy_name = optarg; break;
    ...
    if (benchmark)   run_benchmark(cfg, fb_table, verbose);
    if (interactive) run_interactive(cfg, fb_table, strategy_name);
